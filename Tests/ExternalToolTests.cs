...
namespace Tests
{
    ...
    public sealed class ExternalToolTests : BaseTestClass
    {
        ...
        public async Task ExternalTool_WhenAsync_ConsumingResultWithFailureExitCode_ThrowsException()
        {
            using (var cancel = new CancellationTokenSource(ExecuteTimeout))
            {
                await Assert.ThrowsExceptionAsync<EnvironmentException>(async () => await ExternalTool.ExecuteAsync(TestData.CommandHandler, TestData.MakeCommandString("exit 1"), cancel.Token));
            }
        }
